...
// Insert the functions before `const processJsonImportData` (around line 966)
const insertionPoint = "const processJsonImportData";
if (!code.includes("handleExportMedicinesExcel")) {
  code = code.replace(insertionPoint, functionsToAdd + "\n  " + insertionPoint);
}

// Update the UI
const uiOld = \`<div className="flex justify-between items-center">
                  <div>
...
const uiNew = \`<div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                  <div>
                    <h3 className="font-extrabold text-slate-800 text-base">DATABASE APOTEK ASSYIFA ({medicines.length} Terdaftar)</h3>
                    <p className="text-xs text-slate-400">Tambahkan obat baru atau edit harga katalog obat secara langsung di bawah ini.</p>
                  </div>
